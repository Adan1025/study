                     ...
                     if (data.status == 1) {
                         for (var i = 0; i < arr.length; i++) {
                             str += '<tr><td>' + (i + 1) + '</td><td>' + (arr[i].nickName || '') +
                                 ...
                                 '</td><td>' + (arr[i].metto || '') +
                                 '</td><td>' + '<button type="button" class="edit" data-href="./addUsers.html?id=' + arr[i].id + '"><i class="iconfont icon-bianji"></i></button>' + '<button data-id="' + arr[i].id + '" class="delete">删除</button>'
                             '</td></tr>'
                         }
                         $('#userList').html(str);
                     }
